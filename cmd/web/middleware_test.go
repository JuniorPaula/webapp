...
func Test_application_addIPToContext(t *testing.T) {
	...
	nextHanlder := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		val := r.Context().Value(contextUseKey)
		if val == nil {
			t.Error(contextUseKey, "not present")
		}
	...
	for _, e := range tests {
		...
		handlerToTest := app.addIPToContext(nextHanlder)
...
func Test_application_ipFromContext(t *testing.T) {
	...
	ctx = context.WithValue(ctx, contextUseKey, "whatever")
	...
	if !strings.EqualFold("whatever", ip) {
		t.Error("wrong value return from context")
	}
}
...
func Test_app_auth(t *testing.T) {
	...
	for _, e := range tests {
		handlerTests := app.auth(nextHanlder)
		req := httptest.NewRequest("GET", "http://testing", nil)
