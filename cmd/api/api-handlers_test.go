...
func Test_app_userHandlers(t *testing.T) {
	var tests = []struct {
		...
		{"getUser invalid bag URL param", "GET", "", "Y", app.getUser, http.StatusBadRequest},
		{
			"updateUser valid",
			"PATCH",
		...
		},
		{
			"insertUser invalid",
			"PUT",
			`{"foo": "bar","last_name": "Smith", "email":"jack@example.com"}`,
			"",
			app.insertUser,
			http.StatusBadRequest,
		},
...
func Test_app_refreshUsingCookie(t *testing.T) {
	...
	var tests = []struct {
		...
		{"bad cookie", true, badCookie, http.StatusBadRequest},
	}
