...
func TestMain(m *testing.M) {
	...
	testRepo = &PostgresDBRespo{DB: testDB}
...
func TestPostgresDbRepoGetUser(t *testing.T) {
	...
	if user.Email != "admin@example.com" {
		t.Errorf("wrong returned by GetUser; expected admin@example.com but got %s", user.Email)
	}
...
func TestPostgresDbRepoGetUserByEmail(t *testing.T) {
	user, err := testRepo.GetUserByEmail("jhon@example.com")
	if err != nil {
		t.Errorf("error getting user by email: %s", err)
	}

	if user.ID != 2 {
		t.Errorf("wrong returned by GetUserByEmail; expected 2 but got %d", user.ID)
	}
}
...
func TestPostgresDBRepoUpdateUser(t *testing.T) {
	...
	user, _ = testRepo.GetUser(2)
	if user.FirstName != "Jane" || user.Email != "jane@email.com" {
		t.Errorf("expected updated record to have first name Jane and email jane@email.com, but got %s %s", user.FirstName, user.Email)
	}
}
...
func TestPostgresDbRepoDeleteUser(t *testing.T) {
	...
	if err == nil {
		t.Error("retrieved user id 2, who should been deleted")
	}
}
...
func TestPostgresDBRepoResetPassword(t *testing.T) {
	...
	if !matches {
		t.Errorf("password should match 'password', but got does not")
	}
}
...
func TestPostgresDBRepoInsertUserImage(t *testing.T) {
	...
	if err == nil {
		t.Error("inserted a user image whit non-existent user id")
	}
}
